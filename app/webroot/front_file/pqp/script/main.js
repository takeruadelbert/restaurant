...
$(document).ready(function(){
    ...
    */
    ...
    $(".star.star-picker").each(function(){
        var unitWidth = $(this).outerWidth()/5;
        $(this).attr("data-rating","0");
        $(this).css("background-position",""+(-(unitWidth*5))+"px 0px");
    });

    $(".star.star-picker").mousemove(function(event){
        var unitWidth = $(this).outerWidth()/5;
        var x = event.pageX-$(this).offset().left;
        var leftOffset = 0;
        if (x<unitWidth*1) {
            leftOffset = -(unitWidth*4);
        } else if (x<unitWidth*2) {
            leftOffset = -(unitWidth*3);
        } else if (x<unitWidth*3) {
            leftOffset = -(unitWidth*2);
        } else if (x<unitWidth*4) {
            leftOffset = -(unitWidth*1);
        } else if (x<unitWidth*5) {
            leftOffset = -(unitWidth*0);
        }
        $(this).css("background-position",""+leftOffset+"px 0px");
    });

    $(".star.star-picker").click(function(event){
        var unitWidth = $(this).outerWidth()/5;
        var x = event.pageX-$(this).offset().left;
        var rating = 0;
        if (x<unitWidth*1) {
        ...
        rateProduct(rating, param);
    });

    $(".star.star-picker").mouseout(function(event){
        var unitWidth = $(this).outerWidth()/5;
        var rating = parseInt($(this).attr("data-rating"));
        var multiplier = 5-rating;
        $(this).css("background-position",""+(-(unitWidth*multiplier))+"px 0px");
    });
...
function startDedicatedCounter() {
    $(".dedicated-counter").each(function(){
        currentTime = parseInt((new Date()).getTime()/1000)+timeOffset;
        ...
        var minute = parseInt((timeElapsed-day*3600*24-hour*3600)/60);
        var second = parseInt((timeElapsed-day*3600*24-hour*3600-minute*60));
        $(this).children(".day").children(".number").html(day);
        $(this).children(".hour").children(".number").html(hour);
        $(this).children(".minute").children(".number").html(minute);
        $(this).children(".second").children(".number").html(second);
    });
    setTimeout(function(){
        startDedicatedCounter();
    },200);
...
function loginErrorShow(message) {
    $("#toggle-login-error").removeClass("hidden").find(".error-message").html(message);
}

function loginErrorHide(message) {
    $("#toggle-login-error").addClass("hidden");
}

function rateProduct(rating, param) {
    //IMPLEMENTASI FUNGSI RATING PRODUCT
}
